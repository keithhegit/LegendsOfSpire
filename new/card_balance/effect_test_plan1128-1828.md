...
| :--- | :--- | :--- | :--- | :--- | :--- | ---- |
| **MULTI_HIT** | `JinxQ` | 机枪扫射 | 无 | 打出卡牌 | 造成 3 次伤害，每次显示独立伤害数字；若有力量加成，每次都应生效。 | 只显示一次伤害“24” |
| **VULNERABLE** | `GarenQ` | 致命打击 | 无 | 打出卡牌 | 敌人获得易伤状态（紫色图标），后续攻击伤害 x1.5。 | 没有力量加成？点燃-力量+2，但是技能伤害无增加。 |
| **WEAK** | `TeemoQ` | 致盲吹箭 | 无 | 打出卡牌 | 敌人获得虚弱状态（黄色图标），敌人下回合攻击伤害 x0.75。 | √（还造成6点伤害） |
| **STRENGTH** | `Ignite` | 点燃 | 无 | 打出卡牌 | 玩家力量增加（红色剑图标），下一张攻击牌伤害增加。 | √ |
| **BLOCK** | `Defend` | 防御 | 无 | 打出卡牌 | 玩家获得护甲（蓝色盾条），敌人攻击时优先扣除护甲。 | √（但敌人的护盾效果未生效） |
...
| **GAIN_AGI** | `Neutral_043` | 灵巧身法 | 无 | 打出卡牌 | 玩家获得敏捷（绿色盾图标），每回合开始获得的格挡值增加。 | 未有匹配卡牌技能，后续增加build新卡后再测试 |
| **MARK** | `JinxE` | 火箭陷阱 | 无 | 打出卡牌 | 敌人获得标记（准星图标）。配合触发卡牌（如普通攻击）攻击该敌人。 | 敌人状态栏的标记（准星图标）未产生。额外触发效果也未生效 |
| **MARK_TRIGGER** | `JinxE` | 火箭陷阱 | 敌人有标记 | 攻击敌人 | 触发额外伤害，标记被消耗（或层数减少）。 | 敌人状态栏的标记（准星图标）未产生。额外触发效果也未生效 |
| **CLEANSE** | `GarenW` | 勇气 | 玩家有Debuff | 打出卡牌 | 玩家的一个负面状态（如易伤、虚弱）被移除。 | √ |
| **DOUBLE_HIT** | `VayneQ` | 翻滚 | 无 | 打出卡牌 | 下一次攻击造成双倍伤害（需配合攻击牌验证）。 | 未遇见 |
| **STUN** | `ViktorW` | 引力场 | 无 | 打出卡牌 | 敌人获得眩晕状态，跳过下一回合行动。 | 概率性生效。眩晕ui随回合进行不消失。 |
| **PULL** | `ThreshQ` | 死亡判决 | 无 | 打出卡牌 | 敌人被拉近（视觉效果），某些近战卡牌可能享受加成。 | 造成8伤，无其他效果。 |
| **EXECUTE_SCALE** | `GarenR` | 德玛西亚正义 | 敌人残血 | 打出卡牌 | 造成大量真实伤害，伤害值随敌人已损生命值增加。 | 未遇见|
| **BLEED** | `DariusQ` | 大杀四方 | 无 | 打出卡牌 | 敌人获得流血层数（血滴图标），回合结束受到层数x2的伤害。 | √ |
| **BLEED_VULN** | `DariusE` | 无情铁手 | 无 | 打出卡牌 | 敌人同时获得流血和易伤状态。 | √ |
| **DRAW_NEXT** | `LuxW` | 结界护盾 | 无 | 打出卡牌 | 结束回合，下回合开始时额外抽 1 张牌。 | 未生效。 |
| **BONUS_PER_EXTRA_MANA** | `LuxE` | 透光奇点 | 有额外法力 | 打出卡牌 | 伤害值高于面板，根据当前额外法力值计算加成。 | √ |
| **CONDITIONAL_DOUBLE** | `LuxR` | 终极闪光 | 本回合出牌≥4 | 打出卡牌 | 伤害翻倍（需先打出 3 张其他牌）。 | 未遇见 |
| **LOW_HP_BONUS** | `JinxR` | 超究极飞弹 | 敌人HP<50% | 打出卡牌 | 造成额外伤害（需先将敌人打至半血以下）。 | 未遇见 |
...
## 🛠️ Batch 1 修复报告 (Batch 1 Fix Report)

| 效果 ID / 卡牌 | 问题分析 (Analysis) | 处理方案 (Solution) | 复测建议 (Retest Suggestion) | 复测结果 |
| :--- | :--- | :--- | :--- | ---- |
| **MULTI_HIT** (JinxQ) | 伤害数字重叠显示，导致只看到最后一次或总和，视觉反馈不佳。 | 在 `BattleScene.jsx` 中使用 `setTimeout` 对多段伤害的飘字进行错峰显示 (间隔 300ms)。 | 观察 JinxQ 是否依次弹出 3 个伤害数字 (如 8, 8, 8)。 | 已生效 |
| **VULNERABLE** (GarenQ) | 易伤状态更新与伤害计算在同一帧，导致当次攻击未能享受到易伤加成 (React 状态异步更新)。 | 在 `BattleScene.jsx` 中，提前合并 `effectUpdates` 中的状态变化，确保伤害计算使用最新的状态。 | 先打出 GarenQ，观察是否造成了易伤加成后的伤害 (如 8 * 1.5 = 12)，或后续攻击是否正确加成。 | 已生效 |
| **BLOCK** (Defend) | 敌人回合结束时清空了所有格挡，导致敌人获得的格挡 (BUFF) 在玩家回合开始前就消失了。 | 修改 `BattleScene.jsx` 中的格挡清除逻辑，移除了 `enemyAction` 中错误的 `setEnemyBlock(0)`，保留回合结束时的清除。 | 观察敌人施放格挡技能后，护甲条是否正确保留到玩家回合开始。 | 未找到有当个buff的敌人 |
| **MARK** (JinxE) | 代码中属性名不一致，`cardEffectHandler.js` 使用 `marked`，而渲染层使用 `mark`。 | 统一将属性名修改为 `mark`。 | 打出 JinxE，观察敌人状态栏是否出现准星图标。 | 未出现准星，卡牌标记效果也未生效 |
| **GAIN_AGI** (Neutral_043) | 敏捷 (`Dexterity`) 属性虽然增加了，但在计算格挡时未被累加。 | 在 `BattleScene.jsx` 的格挡计算公式中加入 `playerStatus.dexterity`。 | 打出灵巧身法后，再打出防御牌，观察获得的护甲是否增加了 1 点。 | 厄加特 铁壁守势  敏捷+1 产生了状态栏灵巧图标，也+25防御生效了。 但是没有在次回合产生+防御效果，灵巧的图标一直在状态栏没有消失 |
| **STUN** (ViktorW) | 眩晕状态仅在数据层生效，未阻止敌人行动。 | 在 `BattleScene.jsx` 的 `enemyAction` 中添加检测，若 `stunned > 0` 则跳过攻击逻辑并减少层数。 | 对敌人施加眩晕后，观察敌人是否跳过了攻击回合 (头顶显示 STUNNED)。 | 已生效 |
| **Hero Skills** (VayneQ/GarenR/LuxR) | 部分英雄技能的特殊效果 (`NEXT_ATTACK_BONUS`, `EXECUTE_SCALE`, `CONDITIONAL_DOUBLE`) 未在前端实现逻辑。 | 在 `BattleScene.jsx` 的伤害计算流程中补充了这些效果的具体实现逻辑。 | 分别测试 VayneQ (下一次攻击+3), GarenR (残血斩杀), LuxR (出牌数>=4 翻倍) 是否生效。 | LuxR (出牌数>=4 翻倍) 技能未遇见过，可能未实装？分别测试 VayneQ (下一次攻击+3)已生效。GarenR (残血斩杀)卡牌未出现，请检查是否实装。 |
| **DRAW_NEXT** (LuxW) | 下回合抽牌逻辑未对接。 | 在 `cardEffectHandler.js` 中映射 `DRAW_NEXT` 到 `nextDrawBonus` 状态，并在 `BattleScene.jsx` 回合开始时处理。 | 打出 LuxW，观察下回合开始时是否多抽了 1 张牌。 | 次回合额外抽卡+1已生效，但牌面+6防御未生效 |
