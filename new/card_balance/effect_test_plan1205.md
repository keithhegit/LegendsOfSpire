...
| :--- | :--- | :--- | :--- | :--- | :--- | ---- |
| **TEMP_MANA** | `Neutral_010` | 法力容器 | 无 | 打出卡牌 | 获得临时法力（本回合可用）。 | 已生效 |
| **GAIN_GOLD** | `Neutral_011` | 贪婪商券 | 无 | 打出卡牌 | 金币数量增加。 | 已生效 |
| **CONDITIONAL_DRAW** | `Neutral_012` | 战争学识 | 出2张攻击牌 | 打出卡牌 | 抽 2 张牌（未满足条件抽 1 张）。 | √ |
| **NEXT_DRAW_PLUS** | `Neutral_013` | 战术速记 | 无 | 打出卡牌 | 结束回合。下回合开始时手牌数增加。 | √ |
| **SCOUT** | `Neutral_014` | 暗影笔记 | 无 | 打出卡牌 | 查看敌人下回合意图（如果是隐藏状态则显示）。 | 已生效                                                       |
| **GAMBLE** | `Neutral_015` | 金币押注 | 无 | 打出卡牌 | 随机：获得金币 或 失去生命。 | 已生效 |
| **MANA_IF_LOW_HAND** | `Neutral_016` | 法力石碎片 | 手牌<=3 | 打出卡牌 | 获得法力并抽牌。 | ✅ 打出时若手牌（含本牌）≤3，立即抽 1 并返还 1 点法力，可与其他回蓝效果叠加。 |
| **PERMA_DRAW_ON_KILL** | `Neutral_017` | 求知铭文 | 击杀敌人 | 打出卡牌 | 在战斗中击杀敌人，下轮战斗的首回合多抽1张牌 | 已生效 |
| **PERMA_UPGRADE_CARD** | `Neutral_018` | 匠魂之锤 | 手有可升级牌 | 一次性卡牌 | 选择一张牌，该牌数值永久提升（本局游戏）。 | 已生效 本战斗仅能使用一次:永久升级一张攻击/技能牌(附带即时升级) 效果 变更为： 随机让《打击》或《防御》卡牌的数值永久+1 (如打击从6攻击变为7攻击)，立即生效，和永久力量HP一样。用户在背包查看，应该看到 打击打牌的牌面是 造成7伤害。这里是实装卡牌数值永久成长的机制。 |
| **WIN_GOLD_BONUS** | `Neutral_019` | 荣誉奖章 | 无 | 打出卡牌 | 战斗胜利结算时，金币奖励增加。 | 已生效 |
| **TEMP_STR_ON_KILLS** | `Neutral_020` | 猎手徽章 | 打出徽章 | 一次性卡牌 | 若击败当前敌人则永久 +1 力量。 | 已生效。为了平衡数值，削弱这个强卡。改变效果为该卡牌是一次性卡牌，使用后永久从背包移除，直到下一次用户获取，才能使用第二次，第二次使用后依然会被移除如此类推 |
| **PERMA_STR_FOR_HP** | `Neutral_021` | 野心契约 | 无 | 一次性卡牌 | 牺牲5最大生命 获取永久+1力量，战斗结束后生效，每场战斗仅生效一次。 | 已生效 |
| **DRAW_AT_START** | `Neutral_022` | 古老碑铭 | 无 | 打出卡牌 | 获得能力：每回合开始额外抽牌。 | ✅ 抽牌加成写入 `extraDrawPerTurn`，回合开始额外抽牌。 |
| **RECYCLE** | `Neutral_023` | 逆向引擎 | 弃牌堆有牌 | 打出卡牌 | 随机将本轮已经打出过的1张卡牌重新回到手牌 | 已生效 |
| **SELF_HP_FOR_BLOCK** | `Neutral_024` | 献祭之环 | 无 | 打出卡牌 | 扣除生命值，获得大量护甲。 | ✅ 扣除 6 HP（保留至少1）并额外给 `blockGain`，护甲会即时叠加。 |
| **DISCOUNT_ONE_CARD** | `Neutral_025` | 随风信笺 | 手牌有高费 | 打出卡牌 | 手中随机一张牌费用 -1（本回合）。 | 已生效。 |
| **UPGRADE_CARD** | `Neutral_026` | 暗金卷轴 | 手有普通牌 | 打出卡牌 | 手中一张牌升级（数值提升）。 | 已生效，修改为 "打出后卡包里的一张打击和防御，各自进化为升级版，直到本场战斗结束" |
| **BAIT_TRIGGER** | `Neutral_027` | 毒饵之箱 | 无 | 打出卡牌 | 放置诱饵。敌人攻击时触发8层中毒状态。 | 已生效，变更：3点Mana, 放置诱饵。敌人攻击时触发8层中毒状态。 复用提莫陷阱代码：在敌人行动前触发毒，HUD 显示“TRAP!”。 |
| **NEXT_ENEMY_COST_PLUS** | `Neutral_028` | 回旋弹 | 无 | 打出卡牌 | 造成9伤并使敌人下一张卡费用<br/>+1(资源压制)。。 | 已生效。变更为，造成9伤并使敌人下一张卡费用<br/>+1(资源压制)。 |
| **STR_DEBUFF** | `Neutral_029` | 虚弱契约 | 无 | 打出卡牌 | 敌人攻击减少（施加 3 层虚弱）。 | 已生效。直接施加 3 层虚弱。 |
...
| 坚毅咏叹调   | 恢复8生命。                                                  | √                                                            |
| 喑影虹吸     | 造成7点伤害，并回复5点生命。                                 | √                                                            |
| 蓄势待发     | 每回合你的第一张攻击牌额外造成4点伤害                        | 待修复                                                       |
| 灵魂烈焰     | 对目标造成10伤并减少其力量2。                                | 待修复（未减力量）                                           |
...
| 集火屏障     | 获得15护甲。                                                 | √                                                            |
| 腐蚀炸弾     | 对目标造成7点伤害。                                          | √                                                            |
| 影刃         | 造成6伤并使目标减速1回合。                                   | 待修复（未减速）                                             |
| 手里剑       | 对目标造成三段伤害，总计9点。                                | √                                                            |
| 影分身       | 召唤影分身:在本回合复制下一次对单...                         | 未生效（待实现）                                             |
| 守护之盾     | 1mana=10盾                                                   | 已生效。                                                     |
| 光之束缚     | 对单体造成9点伤害并施加1层易伤。                             | 待修复（当前仅8伤）                                          |
| 凶刃连舞     | 连续3次造成6点伤害。                                         | √                                                            |
| 致残打击     | 造成5点伤害并给予目标1层易伤;本...                           | √；但其他效果未知。                                          |
| 灵魂压制     | 移除目标的一个增益状态。                                     | 难判断。                                                     |
| 罪恶引       | 使目标虚弱2。                                                | √                                                            |
| 破甲一击     | 造成12点伤并减少目标护甲6点。                                | 待验证                                                       |
| 灵魂灯笼     | 恢复8生命。                                                  | √                                                            |
| 强制斩击     |                                                              |                                                              |
...
| 缚魂钩       | 对目标造成5伤并在其上放置“牵绊”标记:若你下回合对其攻击则额外触… | 未生效（待实现）                                             |
| 缠绕陷阱     | 在目标上放置陷阱标记：若目标下回合尝试攻击或打牌，则触发对该目... | 未生效（待实现）                                             |
| 致盲烟幕     | 造成4点伤并使目标虚弱2。                                     | √                                                            |
| 弧刃斩       | 对目标进行三段攻击，总计9伤。                                | √                                                            |
