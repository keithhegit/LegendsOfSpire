...
| **DOUBLE_HIT** | `VayneQ` | 翻滚 | 无 | 打出卡牌 | 下一次攻击造成双倍伤害（需配合攻击牌验证）。 | 就像我和她一样，难以遇见... |
| **STUN** | `ViktorW` | 引力场 | 无 | 打出卡牌 | 敌人获得眩晕状态，跳过下一回合行动。 | 概率性生效。眩晕ui随回合进行不消失。 |
| **PULL** | `ThreshQ` | 死亡判决 | 无 | 打出卡牌 | 敌人被拉近（视觉效果），某些近战卡牌可能享受加成。 | 造成8伤，无其他效果。 |
| **EXECUTE_SCALE** | `GarenR` | 德玛西亚正义 | 敌人残血 | 打出卡牌 | 造成大量真实伤害，伤害值随敌人已损生命值增加。 | 就像我和她一样，难以遇见... |
...
| **CONDITIONAL_DOUBLE** | `LuxR` | 终极闪光 | 本回合出牌≥4 | 打出卡牌 | 伤害翻倍（需先打出 3 张其他牌）。 | 就像我和她一样，难以遇见... |
| **LOW_HP_BONUS** | `JinxR` | 超究极飞弹 | 敌人HP<50% | 打出卡牌 | 造成额外伤害（需先将敌人打至半血以下）。 | 就像我和她一样，难以遇见... |
...
| **BLOCK** (Defend) | 敌人回合结束时清空了所有格挡，导致敌人获得的格挡 (BUFF) 在玩家回合开始前就消失了。 | 修改 `BattleScene.jsx` 中的格挡清除逻辑，移除了 `enemyAction` 中错误的 `setEnemyBlock(0)`，保留回合结束时的清除。 | 观察敌人施放格挡技能后，护甲条是否正确保留到玩家回合开始。 | ⚠️ 未能复测——当前敌人池缺少自带格挡技能；需在 Batch2 Phase2.1 中新增可验证敌人或模拟战斗。 |
| **MARK** (火箭陷阱) | 标记层数未同步到 UI，导致无法确认状态是否存在。 | 统一 `MARK` 数据管道，`BattleScene` 在计算前合并 `enemyStatus` 并渲染准星图标。 | 打出火箭陷阱，检查敌人状态栏是否出现标记层数。 | 已生效 |
| **MARK_TRIGGER** (火箭陷阱) | 标记触发伤害未记录，攻击后没有额外伤害/层数消耗。 | `MARK_TRIGGER` 现在写入 `enemyStatus.markDamage`，`BattleScene` 在每次命中时追加伤害并递减层数。 | 火箭陷阱后用任意攻击命中，观察额外飘字与标记减少。 | 已生效 |
| **GAIN_AGI** (灵巧身法/铁壁守势) | 敏捷 (`Dexterity`) 只在打出当回合生效，无法在下回合提供护甲，图标也不会消失。 | 取消“即时加护甲”写法，转为在 `applyNextTurnEffects` 中按层数+5护甲并清空 `dexterity`。 | 打出灵巧身法或铁壁守势，下一回合开始自动获得护甲并移除灵巧图标。 | 已生效 |
...
| **Hero Skills** (VayneQ/GarenR/LuxR) | 部分英雄技能的特殊效果 (`NEXT_ATTACK_BONUS`, `EXECUTE_SCALE`, `CONDITIONAL_DOUBLE`) 未在前端实现逻辑。 | 在 `BattleScene.jsx` 的伤害计算流程中补充了这些效果的具体实现逻辑。 | 分别测试 VayneQ (下一次攻击+3), GarenR (残血斩杀), LuxR (出牌数>=4 翻倍) 是否生效。 | ⚠️ 仅验证了 VayneQ；GarenR/LuxR 卡牌尚未抽到，需要在 Batch2 Phase2.5 新增测试入口或临时卡牌池确保可复现。 |
| **DRAW_NEXT** (结界护盾) | 抽牌虽已生效，但牌面 +6 护盾未实现。 | 将 `结界护盾` 的护盾值写回 `CARD_DATABASE` 并沿用现有 block 逻辑。 | 打出结界护盾是否立即获得6护甲，且下回合多抽1张。 | 已生效 |
