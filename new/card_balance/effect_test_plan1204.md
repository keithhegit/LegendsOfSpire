...
| :--- | :--- | :--- | :--- | :--- | :--- | ---- |
| **TEMP_MANA** | `Neutral_010` | 法力容器 | 无 | 打出卡牌 | 获得临时法力（本回合可用）。 | ✅ 出牌后立即 +1 临时法力，可在本回合任意使用。 |
| **GAIN_GOLD** | `Neutral_011` | 贪婪商券 | 无 | 打出卡牌 | 金币数量增加。 | ✅ 触发 `GAIN_GOLD` 后即时调用战斗层金币回调，+10G 可在 HUD 立刻看到，且不会再抛错。 |
| **CONDITIONAL_DRAW** | `Neutral_012` | 战争学识 | 出2张攻击牌 | 打出卡牌 | 抽 2 张牌（未满足条件抽 1 张）。 | √ |
| **NEXT_DRAW_PLUS** | `Neutral_013` | 战术速记 | 无 | 打出卡牌 | 结束回合。下回合开始时手牌数增加。 | √ |
| **SCOUT** | `Neutral_014` | 暗影笔记 | 无 | 打出卡牌 | 查看敌人下回合意图（如果是隐藏状态则显示）。 | ✅ 立即弹出“敌人下一动”提示，显示即将执行的行动。 |
| **GAMBLE** | `Neutral_015` | 金币押注 | 无 | 打出卡牌 | 随机：获得金币 或 失去生命。 | ✅ 50% 概率 +30G，50% 概率失去 10 HP（至少保留 1 HP），同时浮现绿色/红色提示。 |
| **MANA_IF_LOW_HAND** | `Neutral_016` | 法力石碎片 | 手牌<=3 | 打出卡牌 | 获得法力并抽牌。 | ✅ 打出时若手牌（含本牌）≤3，立即抽 1 并返还 1 点法力，可与其他回蓝效果叠加。 |
| **PERMA_DRAW_ON_KILL** | `Neutral_017` | 求知铭文 | 击杀敌人 | 打出卡牌 | 击杀后，永久获得“每回合多抽1牌”的效果。 | ✅ 杀死敌人后，为“下一场战斗”的首回合累积 +1 抽牌（可叠），进入战斗即消耗。 |
| **PERMA_UPGRADE_CARD** | `Neutral_018` | 匠魂之锤 | 牌组含《打击/防御》 | 打出卡牌 | 随机让《打击》或《防御》永久 +1 数值，并即时体现在战斗和牌组显示。 | ✅ 费用 3、Exhaust：随机命中一张《打击/防御》，立即在战斗中加 +1（卡牌面板新增“匠魂+X”标识），并将该卡写回存档保留增益；多次使用会叠加。 |
| **WIN_GOLD_BONUS** | `Neutral_019` | 荣誉奖章 | 无 | 打出卡牌 | 战斗胜利结算时，金币奖励增加。 | ✅ 每次施放都会叠加 `winGoldBonus`，战斗胜利后自动额外结算 +10G 并弹出金色 Toast。 |
| **TEMP_STR_ON_KILLS** | `Neutral_020` | 猎手徽章 | 打出徽章 | 打出卡牌 | 若击败当前敌人则永久 +1 力量。 | ✅ 卡牌标记为一次性：打出即从本战斗中 Exhaust，胜利后永久 +1 力量并从背包中移除（需重新获得才可再次使用）。 |
| **PERMA_STR_FOR_HP** | `Neutral_021` | 野心契约 | 无 | 打出卡牌 | 扣除最大生命值，永久获得力量。 | ✅ 每场战斗仅能触发一次：立即记录 -5 Max HP，战斗胜利后永久 +1 力量并提示。 |
| **DRAW_AT_START** | `Neutral_022` | 古老碑铭 | 无 | 打出卡牌 | 获得能力：每回合开始额外抽牌。 | ✅ 抽牌加成写入 `extraDrawPerTurn`，回合开始额外抽牌。 |
| **RECYCLE** | `Neutral_023` | 逆向引擎 | 弃牌堆有牌 | 打出卡牌 | 随机将弃牌堆的一张牌移回手牌。 | ✅ 随机选择本战斗已打出的 1 张牌回到手牌，并显示“回收 XXX”提示。 |
| **SELF_HP_FOR_BLOCK** | `Neutral_024` | 献祭之环 | 无 | 打出卡牌 | 扣除生命值，获得大量护甲。 | ✅ 扣除 6 HP（保留至少1）并额外给 `blockGain`，护甲会即时叠加。 |
| **DISCOUNT_ONE_CARD** | `Neutral_025` | 随风信笺 | 手牌有高费 | 打出卡牌 | 手中随机一张牌费用 -1（本回合）。 | ✅ 立即在被选中卡牌的法力球旁出现闪烁 “-1” 标记；出牌或回合结束时折扣消失，HUD 与费用扣除一致。 |
| **UPGRADE_CARD** | `Neutral_026` | 暗金卷轴 | 手有普通牌 | 打出卡牌 | 手中一张牌升级（数值提升）。 | ✅ 仅针对《打击 / 防御》进行战斗期升级（+3 数值），不会写回永久牌组。 |
| **BAIT_TRIGGER** | `Neutral_027` | 诱饵之箱 | 无 | 打出卡牌 | 放置诱饵。敌人攻击时触发反击伤害。 | ✅ 复用提莫陷阱：在敌人行动前触发毒+虚弱，HUD 显示“TRAP!”。 |
| **NEXT_ENEMY_COST_PLUS** | `Neutral_028` | 回旋弹 | 无 | 打出卡牌 | 敌人下回合行动（如果是出牌）费用增加（对AI通常表现为行动力受限）。 | √ |
| **STR_DEBUFF** | `Neutral_029` | 虚弱契约 | 无 | 打出卡牌 | 敌人力量减少（攻击伤害降低）。 | ✅ 直接施加 3 层虚弱。 |
| **TRANSFORM** | `Neutral_030` | 重塑石 | 手有牌 | 打出卡牌 | 手中一张牌变为另一张随机牌。 | ✅ 随机将一张手牌替换为背包/牌组中已有的其他卡。 |
| **REGEN_MANA** | `Neutral_035` | 能量回复 | 无 | 打出卡牌 | 本回合和下回合各恢复法力。 | ✅ 当前 +1 法力并叠加 `nextTurnMana`，下回合自动回蓝。 |
| **PASSIVE_BLOCK_IF_IDLE** | `Neutral_036` | 稳固姿态 | 不出其他牌 | 结束回合 | 下回合开始获得护甲。 | ✅ 若本回合只使用该牌直接结束，则在下一回合开始获得 effectValue 护甲。 |

> 术语说明：此处“弃牌堆”指本轮战斗中已打出并进入弃牌区的卡牌集合。
...
| **CRIT_IF_VULN** | `Neutral_042` | 精准打击 | 敌人易伤 | 打出卡牌 | 必定暴击（伤害增加）。 | 造成8点伤害；若目标处于易伤，额外造成6点伤害。 |
| **NEXT_ATTACK_X2** | `Neutral_044` | 集火号令 | 无 | 打出卡牌 | 下一张攻击牌伤害 x2。 | 未生效。 |
| **GAIN_STRENGTH_PER_HIT** | `Neutral_045` | 积蓄怒火 | 无 | 打出卡牌 | 获得能力：每次受击，力量 +1。 | 未生效。 |
| **PASSIVE_BLOCK** | `Neutral_047` | 诱敌深入 | 无 | 打出卡牌 | 给目标施加“诱捕标记":目标下次攻击你时受到8点反噬。 | 生成“标记”标志，但未实际效果。 |
| **BLOCK_DRAW** | `Neutral_055` | 不屈意志 | 无 | 打出卡牌 | 获得12点护甲并移除1个负面状态。 | 移除1个负面状态，但未获得护盾 |
| **GAIN_MANA** | `Neutral_058` | 军备补给 | 无 | 打出卡牌 | 抽3张牌。 | √ |
| **RETALIATE_ON_HIT** | `Neutral_034` | 逆伤反震 | 无 | 打出卡牌 | 获得反伤状态。受击时敌人扣血。 | 未生效。 |
| **NEXT_DAMAGE_REDUCE** | `Neutral_033` | 格挡之歌 | 无 | 打出卡牌 | 获得减伤状态。下一次受击伤害减少。 | 未生效。 |
| **PERMA_STR_ON_KILL** | `NasusQ` | 汲魂痛击 | 击杀敌人 | 打出卡牌 | 击杀后，永久力量 +1。 | 力量未+1，但有力量+1的提示框。 |
| **WEAK_VULN** | `NasusW` | 腐化咒 | 无 | 打出卡牌 | 敌人同时获得虚弱和易伤。 | √ |
| **KILL_REWARD** | `IreliaQ` | 利刃冲击 | 击杀敌人 | 打出卡牌 | 击杀后，返还法力并抽牌。 | 击杀以后，怎么获得法力和卡牌。 |
| **REFLECT_BUFF** | `IreliaW` | 反击之舞 | 无 | 打出卡牌 | 获得12护甲：若被命中则下一次攻击伤害x2。 | 未生效。 |
...
| **JinxPassive** | (被动) | 罪恶快感 | 击杀/空手 | 击杀或打空手牌 | 获得能量或抽牌奖励。 | 击杀敌人后，怎么获得收益。 |
| **YasuoPassive** | (被动) | 浪客之道 | 移动/出牌 | 观察护甲 | 暴击率翻倍；移动（出牌）积攒护甲。 | 出牌，并未添加护盾，伤害未加成。 |
| **SonaPassive** | (被动) | 能量和弦 | 施放3次技能 | 使用3次技能 | 下一次普攻附带额外效果（伤害/减伤/减速）。 | 普攻是“打击”的话，未生效。 |
| **EkkoPassive** | (被动) | Z型驱动 | 3次攻击 | 攻击3次 | 第3次攻击造成额外伤害并偷取移速（敏捷）。 | 未生效。 |
| **SylasPassive** | (被动) | 破敌禁法 | 施放技能 | 使用技能牌 | 下一次普攻变为范围伤害（AOE）或强化普攻。 | 未生效。 |
| **UrgotPassive** | (被动) | 回响烈焰 | 攻击方向 | 观察伤害 | 周期性对周围敌人造成额外伤害（卡牌游戏中可能体现为周期性AOE）。 | 攻击方向是什么 |
...
| **NasusPassive** | (被动) | 吞噬灵魂 | 攻击 | 使用攻击牌 | 获得生命偷取（回血）。 | 未生效。 |
| **IreliaPassive** | (被动) | 艾欧尼亚热诚 | 技能命中 | 使用技能牌 | 获得攻速（多抽牌）和护盾伤害加成。 | 未生效。 |
| **KatarinaPassive** | (被动) | 贪婪 | 击杀 | 击杀敌人 | 技能冷却刷新（回手或减费）。 | 击杀敌人后，怎么有技能？ |
...
| 坚毅咏叹调   | 恢复8生命。                                                  | √                                                            |
| 喑影虹吸     | 造成7点伤害，并回复5点生命。                                 | √                                                            |
| 蓄势待发     | 每回合你的第一张攻击牌额外造成4点伤害                        | 未生效。                                                     |
| 灵魂烈焰     | 对目标造成10伤并减少其力量2。                                | 造成10伤，但力量未减2。                                      |
| 致命连击     | 造成10点伤害，然后再次造成10点伤害。                         | √                                                            |
|              |                                                              |                                                              |
|              |                                                              |                                                              |
|              |                                                              |                                                              |
|              |                                                              |                                                              |
...
| 集火屏障     | 获得15护甲。                                                 | √                                                            |
| 腐蚀炸弾     | 对目标造成7点伤害。                                          | √                                                            |
| 影刃         | 造成6伤并使目标减速1回合。                                   | 造成6伤，但未减速1回合。                                     |
| 手里剑       | 对目标造成三段伤害，总计9点。                                | √                                                            |
| 影分身       | 召唤影分身:在本回合复制下一次对单...                         | 未生效。                                                     |
| 守护之盾     | 1mana=10盾                                                   | 未生效。                                                     |
...
| **GAMBLE** | Neutral_015 | 随机金币/扣血 | 需要随机逻辑 + 金币系统 | ✅ P3（已实现 50/50 + 提示） |
...
**状态**: ✅ `PERMA_STR_FOR_HP` 会在 `battleResult` 里带出 `maxHpCost`，`handleBattleWin` 先增长再扣除后，界面会弹出“生命代价”；`SELF_HP_FOR_BLOCK` 读入 `blockGain` 并立即加护甲；`TEMP_MANA` 实际算入 `manaChange` 并立即更新 `playerMana`。
...
**状态**: ✅ `MANA_IF_LOW_HAND` 已在 `playCard` 里按打出后手牌数判断返法力；`REGEN_MANA` 立刻 +1 法力并写入 `nextTurnMana`，下回合自动回蓝。
...
**状态**: ✅ `DRAW_AT_START` 已经将加抽写入 `extraDrawPerTurn` 且 `startTurnLogic` 额外抽牌（+1 默认）；`STR_DEBUFF` 现改为直接施加 3 层虚弱，战斗内可见。
...
1. **PASSIVE_BLOCK_IF_IDLE** (Neutral_036) - 20分钟
   ```javascript
   // BattleScene.jsx
   const [idleBlockState, setIdleBlockState] = useState({ value: 0, armed: false, broken: false });
   // 播放稳固姿态时
   setIdleBlockState({ value: effectValue, armed: true, broken: false, sourceCard: cardId });
   // 若后续再打出其他牌则 set ...broken = true
   // endTurn
   if (idleBlockState.armed && !idleBlockState.broken) {
...
**状态**: ✅ 若本回合仅使用稳固姿态直接结束，下回合开始自动获得 `effectValue` 护甲。
