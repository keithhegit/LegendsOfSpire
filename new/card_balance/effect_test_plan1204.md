...
| **PERMA_STR_FOR_HP** | `Neutral_021` | 野心契约 | 无 | 打出卡牌 | 扣除最大生命值，永久获得力量。 | ✅ 力量依旧+value，战斗结束时扣除 `maxHpCost` 并提示生命代价。 |
| **DRAW_AT_START** | `Neutral_022` | 古老碑铭 | 无 | 打出卡牌 | 获得能力：每回合开始额外抽牌。 | ✅ 抽牌加成写入 `extraDrawPerTurn`，回合开始额外抽牌。 |
| **RECYCLE** | `Neutral_023` | 逆向引擎 | 弃牌堆有牌 | 打出卡牌 | 将弃牌堆的一张牌移回手牌。 | ✅ 将弃牌堆（本战斗已打出的牌）顶牌回收到手牌并提示。 |
