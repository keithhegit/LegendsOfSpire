...
| **PERMA_UPGRADE_CARD** | `Neutral_018` | 匠魂之锤 | 手有可升级牌 | 打出卡牌 | 选择一张牌，该牌数值永久提升（本局游戏）。 | ✅ 费用 3、使用后 Exhaust：自动升级第一张攻击/技能牌（含当前战斗和存档），每次使用都会永久写入存档。 |
| **WIN_GOLD_BONUS** | `Neutral_019` | 荣誉奖章 | 无 | 打出卡牌 | 战斗胜利结算时，金币奖励增加。 | ✅ 每次施放都会叠加 `winGoldBonus`，战斗胜利后自动额外结算 +10G 并弹出金色 Toast。 |
| **TEMP_STR_ON_KILLS** | `Neutral_020` | 猎手徽章 | 打出徽章 | 打出卡牌 | 若击败当前敌人则永久 +1 力量。 | ✅ 使用后只要本局胜利就永久 +1 力量，结算面板会提示。 |
...
| **DISCOUNT_ONE_CARD** | `Neutral_025` | 随风信笺 | 手牌有高费 | 打出卡牌 | 手中随机一张牌费用 -1（本回合）。 | ✅ 当回合随机锁定一张手牌 -1 费，打出或回合结束即失效。 |
