...
1. **入口**：右下角新增 “背包” 按钮，点击后右侧会滑出浮层（支持战斗中查看）。
2. **卡牌牌组 Tab**：展示当前卡组明细，顶部仍有 GM 绿色 Banner（注入/起手卡清单），可用于复核 QA 配置。
3. **装备 / 遗物 Tab**：列出英雄被动与当前获取的遗物，后续扩展装备位也会放在这里。
4. **英雄被动 Tab**：集中展示英雄基础属性、简介、GM 备注，便于截图记录。
5. **关闭方式**：点击面板右上角 X 或点击面板外区域即可收起。
