...
2. [卡牌数据](#卡牌数据) - 200张卡牌（120张中立卡 + 80张英雄专属技能）
...
> **说明**：v0.9.1 将通用卡扩展至 **120 张**。完整 JSON 可在 `src/data/cards.js` 的 `NEUTRAL_CARD_POOL` 中查看；下表仅保留设计稿阶段的代表性示例，便于核对格式与字段。
...
> **英雄专属技能总览**：20 位英雄 × 4 张（Q/W/E/R）= 80 张。以下仍保留逐英雄的设计稿条目，方便与 `CHAMPION_CARD_MAP` 对照；如需批量导出请运行 `node scripts/exportCards.mjs`。
...
### R 技能获取方式（cards.js + Batch 1-6 设计）

- **战斗奖励**：所有 R 技能均标记为 `RARE`，遵循上表 20%/槽位的掉落率（来源：`cards.js` 与 `effect_test_plan1130.md`）。
- **商店**：稀有池刷新时同样会出现 R 技能（参考“卡牌稀有度说明”：RARE=金色=终结技），价格集中在 150G 左右。
- **事件/掉落**：目前没有额外产出；若事件中提及“终结技”奖励，则复用同一稀有卡池（Batch 5 以后如需自定义，再扩展本小节）。
- **版本策略**：Batch 2~3 的修复路线已把“大招”效果纳入优先级列表（详见 `effect_test_plan1130.md` 新增的 R 技能稽核表），建议所有英雄至少提供 1 张 R 技能以维持 20 英雄的稀有卡一致性。
...
### 卡牌总数统计
- **中立卡**：120张  
  - BASIC：12张（全新起始卡池，覆盖攻击/防御/加成）  
  - COMMON：40张（普适攻击与功能卡）  
  - UNCOMMON：48张（事件/商店掉落为主）  
  - RARE：20张（高阶功能卡）
- **英雄专属技能**：80张（20位英雄 × Q/W/E/R）
- **总计**：200张（最新数据同步自 `src/data/cards.js`，构建脚本自动生成）
...
### 版本历史
- **v0.7.5** (2025-11-20): 初始版本，包含10位英雄、42张卡牌、8个敌人
- **v0.8.0** (2025-11-21):
  - ✨ 全英雄实装：新增10位英雄 (瑞文、卡牌、盲僧、薇恩、提莫、劫、内瑟斯、艾瑞莉娅、锤石、卡特琳娜)
  - ✨ 永久成长机制 + 被动技能全量上线
  - 🎮 总计：20位英雄、82张卡牌、30个遗物
- **v0.9.1** (2025-11-26) ⭐ **当前版本**:
  - 🔄 **卡牌库翻倍**：中立卡扩充至120张，英雄专属技能80张，总量200张。
  - 🧠 **数据分层**：中立卡 / 英雄卡分离，配合 `authService` 的 per-user 存档使牌库解锁与账号绑定。
  - 🛰️ **文档同步**：本文件与 `PROJECT_DOCUMENTATION.md`、`wrangler.toml` 的 D1 接入方案保持一致。

---

> **提示**：本文档基于 `v0.9.1` (main/new_ui 分支) 生成，数据同步自最新 `src/data/*.js` 并与线上 Pages 环境一致。如有变更请参考源代码或访问 https://lol.keithhe.com 。
